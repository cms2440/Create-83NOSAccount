...
I'm also really fucking lazy and want to minimize repetitive effort.
...
$UserOU = Get-ADOrganizationalUnit -SearchBase $basesOU -Filter {Name -eq "NOSC Users"} | select -ExpandProperty DistinguishedName
$ACCAdminOU = "OU=NOSC,OU=Administrative Accounts,OU=Administration,DC=acc,DC=accroot,DC=ds,DC=af,DC=smil,DC=mil"
...
"@.split("`n") | foreach {$_.trim()}
    :main foreach ($field in $Fields) {
        ...
        elseif ($Field -match "Accounts to Create") {
            ...
            if ($Area42Creds -or $usafeCreds -or $AfnoCreds) {
                [void] $DataEntryHash[$Field].Items.Add("USAFE Admin(s)")
                }
            
            $DataEntryHash[$Field].Height = ($CharHeight + $LabelBuffer) * $DataEntryHash[$Field].Items.count

            $form.Controls.Add($DataEntryHash[$Field]) 

            $OffsetY += $DataEntryHash[$Field].Height + $ComboBuffer
            }
        #Every other item is manual entry, so just use Textboxes
            ...
            switch -Regex ($Field) {
                ...
                "City" {$DataEntryHash[$Field].Text = "NOSC"} #Used to be Langley AFB, but is now NOSC for the logon script.
    ...
    #Gui to see if we already have a user account
    #We'll just base the admin sam off the user
    #List the accounts with the first and last names in NOSC OU, and ask if one of them is ours (IF there are any)
    $EDIPIFilter = $EDIPI + "*"
    [array]$PotentialAccounts = Get-ADUser -Server $ACCDC -SearchBase $UserOU -Filter {GivenName -eq $FirstName -and Surname -eq $Lastname -and UserPrincipalName -like $EDIPIFilter} -Properties DisplayName | select DisplayName,samaccountname
    if ($PotentialAccounts.count -gt 0) {
        do {
            $OffsetY = 20
            
            #Find the longest displayname
            $MaxWidth = 0
            foreach ($str in $PotentialAccounts) {
                $StrPixelLength = [System.Windows.Forms.TextRenderer]::MeasureText($str.DisplayName, $font).width
                if ($StrPixelLength -gt $MaxWidth) {$MaxWidth = $StrPixelLength}
                }
            $MaxWidth = [math]::Max($MaxWidth,325)
            $MaxWidth += $OffsetX * 3
            
            $form_AccountFound = New-Object System.Windows.Forms.Form 
            $form_AccountFound.Text = "Accounts that already Exist"
            $form_AccountFound.StartPosition = "CenterScreen"
            
            $Label = New-Object System.Windows.Forms.Label
            $Label.Location = New-Object System.Drawing.Point($OffsetX,$OffsetY)
            $Label.Size = New-Object System.Drawing.Point($MaxWidth,(($PotentialAccounts.count + 4) * $CharHeight)) 
            $Label.Text = "Do one of the following Accounts correspond to our current user?`n`n"
            $Label.Text += ($PotentialAccounts | select -ExpandProperty DisplayName) -join "`n"
            $form_AccountFound.Controls.Add($Label)
            
            $OffsetY += ($PotentialAccounts.count + 4) * $CharHeight
            
            $YesButton = New-Object System.Windows.Forms.Button
            $YesButton.Location = New-Object System.Drawing.Point(25,$OffsetY)
            $YesButton.Size = New-Object System.Drawing.Size(75,23)
            $YesButton.Text = "Yes"
            $YesButton.DialogResult = [System.Windows.Forms.DialogResult]::Yes
            $form_AccountFound.AcceptButton = $YesButton
            $form_AccountFound.Controls.Add($YesButton)

            $NoButton = New-Object System.Windows.Forms.Button
            $NoButton.Location = New-Object System.Drawing.Point(100,$OffsetY)
            $NoButton.Size = New-Object System.Drawing.Size(75,23)
            $NoButton.Text = "No"
            $NoButton.DialogResult = [System.Windows.Forms.DialogResult]::No
            $form_AccountFound.Controls.Add($NoButton)

            $CancelButton = New-Object System.Windows.Forms.Button
            $CancelButton.Location = New-Object System.Drawing.Point(175,$OffsetY)
            $CancelButton.Size = New-Object System.Drawing.Size(75,23)
            $CancelButton.Text = "Cancel"
            $CancelButton.DialogResult = [System.Windows.Forms.DialogResult]::Cancel
            $form_AccountFound.CancelButton = $CancelButton
            $form_AccountFound.Controls.Add($CancelButton)
                
            #Dynamically decide how long our window will be
            #I honestly don't remember my logic behind the additions, but it works
            $form_AccountFound.Size = New-Object System.Drawing.Size(($MaxWidth + ($OffsetX * 2)),($OffsetY + $TextBoxHeight*4)) 

            #Make sure our GUI is up in yo face
            $form_AccountFound.Topmost = $True
            
            #If the account already exists, have them select which one it is (dropdown) and get its sam
            if ($form_AccountFound.ShowDialog() -eq "Yes") {
                $OffsetY = 20
                
                $form_SelectAccount = New-Object System.Windows.Forms.Form 
                $form_SelectAccount.Text = "Select account"
                ...
                foreach ($str in $PotentialAccounts) {
                    $ComboBox.Items.Add($str.DisplayName) | Out-Null
                    }
                ...
                if ($form_SelectAccount.ShowDialog() -eq "OK") {
                    if ($ComboBox.SelectedIndex -eq 0) {$LegSAM = ""}
                    #$LegSAM = $PotentialAccounts[$ComboBox.SelectedIndex].samaccountname
                    $UserSAM = $PotentialAccounts[$ComboBox.SelectedIndex].samaccountname
                    }
                else {exit} 
                }            
            #If they say that none of the listed accounts are for this user, then find a free sam
            elseif ($form_AccountFound.ShowDialog() -eq "No") {
                #$LegSAM = Find-FreeSam ($Legsam[0..19] -join "")
                $UserSAM = Find-FreeSam ($EDIPI + $PCC)
                }
            #Cancel means exit script
            else {exit}
            } until ($UserSAM -ne "")
            #$UserSAM = $LegSAM
        }
    #If no accounts found, set our new SAM
    else {
        $UserSAM = $EDIPI + $PCC
        }
    ...
    function Make-Account {
        ...
        if ($type -match "Admin") {
            ...
            $admSelectorForm.Text = "Select ADM(s)"
        ...
        :accounts foreach ($account in $Accounts) {
            ...
            if ($newUser) { 
                ...
                try {
                    ...
                    #Add to string to paste into remedy
                    "$netBIOSDomainName\$($account.sam) already exists and has been reconfigured.`n" | Write-Output
                    }
                catch {
                ...
                #Add to string to paste into remedy
                "$netBIOSDomainName\$($account.sam) has been created`n" | Write-Output
                }
    ...
    #Exit output
    $outputStr -match "^\w+\\1\d{9}" | Write-Host
